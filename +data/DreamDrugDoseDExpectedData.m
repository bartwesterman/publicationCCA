classdef DreamDrugDoseDExpectedData < handle
    ...
    methods
        ...
        function obj = init(obj, thesauri, combinationPath, qualityTableFilePath)
            ...
            dExpected        = zeros(exampleCount, 1);
            exampleId        = zeros(exampleCount, 1);
            obj.drugDoseLethalityTable = table(cellLine, drugDosePairList, dExpected, exampleId);
...
%                 unitDrugB = csvCellMatrix{12, 2};
                ...
                dExpectedMatrix = combenefit.doseMatrixToDExpected(dosageLevelsA, dosageLevelsB, survivalPercentageMatrix);
                
                if (any(any(isnan(dExpectedMatrix))))
                    continue;
                end
                for aIndex = 1:length(dosageLevelsA)
                for bIndex = 1:length(dosageLevelsB)
                    
                    dExpected = dExpectedMatrix(aIndex, bIndex) / 100;
                    ...
                    obj.drugDoseLethalityTable.dExpected(nextExampleIndex)        = dExpected;
...
%             end
        ...
        function propertyValueList = rowToEntityIdValueList(obj, rowIndex)
            ...
            deathRate = obj.drugDoseLethalityTable.dExpected(rowIndex);
        ...
        function [essentialEntityIds, essentialValues, cellLineId, drugAId, drugBId, exampleId]  = getExampleData(obj, exampleIndex)
            dExpected        = obj.drugDoseLethalityTable.dExpected(exampleIndex);
            ...
            essentialValues    = [dExpected;       drugValues; 1         ];
